...
├─────────────────────────────────────────────────────────────┤
│  ┌──────────────┐  ┌──────────┐  ┌──────────┐              │
│  │NavDropdown   │  │Screenshot│  │GameList  │              │
│  │(悬浮下拉导航) │  │Grid      │  │          │              │
│  └──────┬───────┘  └────┬─────┘  └────┬─────┘              │
│         │                │              │                    │
│  ┌──────┴────────────────┴──────────────┴──────┐           │
...
#### NavDropdown.jsx
悬浮下拉菜单导航组件，嵌入各视图页面头部，负责：
- 显示当前视图名称作为触发按钮
- 悬浮时展开包含四个导航选项的下拉菜单
- 当前选中项左侧高亮条标识
- 回收站项显示红色计数徽章
- 平滑过渡动画（0.2s 淡入淡出 + 位移）
- 键盘访问支持（Tab/Enter/Space/Escape/方向键）
- 鼠标离开 300ms 后自动收起
...
### v2.3.0 (2026-04-22)
- 重构导航系统：移除左侧固定侧边栏，替换为悬浮下拉菜单导航 (NavDropdown)
- NavDropdown 嵌入各视图页面头部标题位置，悬浮触发展开
- 重写分页组件 (Pagination)，简化为简约紧凑的5按钮布局
- 修复软删除截图后回收站计数徽章不即时刷新的问题
- 附注检索弹窗支持 Enter 键确认
- 调试面板迁移为右下角固定悬浮面板
- 移除 sidebarCollapsed 状态及相关 localStorage 持久化
